...
Works against a sample file today and the friend's live API later.

Examples:
    python scripts/run_threat_intel_ingest.py --sample docs/sample_incidents.json --no-official
    python scripts/run_threat_intel_ingest.py --since 2026-06-01     # uses THREAT_INTEL_BASE_URL
"""
...
from app.threat_intel import fetch_incidents_from_api, ingest, load_sample, pull_official_coverage
...
def main() -> None:
    ...
    ap.add_argument("--since", help="Only pull incidents since YYYY-MM-DD (live API)")
    ...
    if args.sample:
        ...
        raws = fetch_incidents_from_api(since=args.since, limit=args.limit)
        source = "live feed (THREAT_INTEL_BASE_URL)"
